function TitleResume() {
    ...
    cy.get("#resume_title")
        .should("be.visible")
        .clear()
        .type("Fresher Automation Tester");
...
function PersonalInfo() {
    ...
    for (var i = 0; i < 2; i++) {
        cy.get(".dtpicker-content")
            .find(
                'a[class="dtpicker-compButton increment dtpicker-compButtonEnable"]'
            )
            .eq(0)
            .click();
        cy.get(".dtpicker-content")
            .find(
                'a[class="dtpicker-compButton decrement dtpicker-compButtonEnable"]'
            )
            .eq(1)
            .click();
        cy.get(".dtpicker-content")
...
function CareerInfo() {
    ...
    if (cy.get("#INDUSTRY_ID_chosen") === 0) {
        ...
        cy.get('[class="search-choice"] a[class="search-choice-close"]')
            .eq(0)
            .click();
        cy.get("#INDUSTRY_ID_chosen").click();
        cy.get("#INDUSTRY_ID_chosen")
...
function Reference() {
    ...
    cy.get("#rref_name").clear().type("Ngô Đặng Thái Linh"); // input field Name Surname
    cy.get("#rref_title").clear().type("CEO"); // title field
    cy.get("#rref_company").clear().type("Công Ty truyền và vận tải XNK Dental"); // Name company
    cy.get("#rref_phone").clear().type("0964561234"); // Phone number
    cy.get("#rref_email").clear().type("linh.dng@ceo.dental.com"); // Email address
